...
int ConvertTile(Converter* converter, OpInfo* op, Scope* scope) {
  ...
  if (HasInput(op, scope, "RepeatTimes") && !op->Input("RepeatTimes").empty()) {
		repeats_name = op->Input("RepeatTimes").front();
		repeats_operand = converter->AddInputOperand(scope, repeats_name);
	} else if (HasInput(op, scope, "repeat_times_tensor") && !op->Input("repeat_times_tensor").empty()) {
  ...
  // Tile operation
  converter->AddOperation(NNADAPTER_TILE,
                          {input_operand, repeats_operand},
                          {output_operand});
  return NO_ERROR;
}
