...
class StrideSliceComputeTester : public arena::TestCase {
 protected:
 ...
 public:
  StrideSliceComputeTester(const Place& place,
  ...
  void PrepareOpDesc(cpp::OpDesc* op_desc) {
    ...
    if (use_tensor_) {
      ...
      for (size_t i = 0; i < starts_.size(); ++i) {
        starts_tensor_list_.push_back("starts_tensor_list_" + paddle::lite::to_string(i));
        ends_tensor_list_.push_back("ends_tensor_list_" + paddle::lite::to_string(i));
        strides_tensor_list_.push_back("strides_tensor_list_" + paddle::lite::to_string(i));
      }
