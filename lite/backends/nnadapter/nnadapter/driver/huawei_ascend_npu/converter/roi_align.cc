...
int ConvertRoiAlign(Converter* converter, hal::Operation* operation) {
  ROI_ALIGN_OPERATION_EXTRACT_INPUTS_OUTPUTS
	NNADAPTER_CHECK_EQ(aligned, false) << "ROIAlignNPU only support Aligned attribute equaled to False"
  ...
  if (!rois_operator) {
	...
	auto batch_indices_operator = converter->GetMappedOperator(batch_indices_operand);
  ...
  auto unsqueeze_op = converter->AddOperator<ge::op::Unsqueeze>(output_operand, "unsqueeze");
  unsqueeze_op->set_attr_axes(ge::Operator::OpListInt(std::vector<int32_t>({1})));
  SET_INPUT(unsqueeze_op, x, roi_nums_fp32);
  auto roi_nums_fp32_unsqueeze = MAP_OUTPUT(unsqueeze_op, y, output_operand);
	// Concat to make (N, 5)
	auto concat_op = converter->AddOperator<ge::op::ConcatD>(output_operand, "concat");
  ...
  SET_INPUT(roi_align_op, x, input_operator);
  SET_INPUT(roi_align_op, ROIs_N5, roisN5_operator);
